...
namespace JW8307A.ViewModels
{
    ...
    internal class TestViewMode : NotificationObject
    {
        ...
        {
            testItemList.Clear();
            foreach (string t in Person.TestWave)
            {
                TestItemModel testItemMode = new TestItemModel { Wave = t };
                testItemList.Add(testItemMode);
            }
            Result = string.Empty;
        ...
        {
            TestItemList[0].IlValue = il1Value;
            TestItemList[0].RlValue = rl1Value;
            TestItemList[1].IlValue = il2Value;
            TestItemList[1].RlValue = rl2Value;
            isIl1Pass = GetResult(Convert.ToSingle(TestItemList[0].IlValue), ThreSet[0].IlLower, ThreSet[0].IlUpper);
            isIl2Pass = GetResult(Convert.ToSingle(TestItemList[1].IlValue), ThreSet[0].IlLower, ThreSet[1].IlUpper);
            isRl1Pass = GetResult(Convert.ToSingle(TestItemList[0].RlValue), ThreSet[0].IlLower, ThreSet[0].RlUpper);
            isRl2Pass = GetResult(Convert.ToSingle(TestItemList[1].RlValue), ThreSet[0].IlLower, ThreSet[1].RlUpper);
        ...
        {
            if (string.IsNullOrEmpty(TestItemList[0].IlValue))
            {
        ...
        private void GetDataToDataTable()
        {
            testDataDetailModel = new TestDataDetailModel
            {
                ...
                SerialNumber = Sn,
                Il1 = TestItemList[0].IlValue,
                Rl1 = TestItemList[0].RlValue,
                Il2 = TestItemList[1].IlValue,
                Rl2 = TestItemList[1].RlValue,
                Oper = Person.OpName,
            ...
            if (res == DialogResult.OK)
            {
                ...
                for (int i = 0; i < Person.TestWave.Length; i++)
                {
                    List<string> lst = new List<string>
                    {
                        ...
                        threSet[i].IlLower.ToString(),
                        threSet[i].IlUpper.ToString(),
                        threSet[i].RlLower.ToString(),
                        threSet[i].RlUpper.ToString()
                    };
        ...
        private void CheckServer()
        {
            ...
            cmdText = "SELECT ItemTestname FROM dbo.Test Where EndDateTime=(SELECT TOP 1 dbo.Test.EndDateTime From dbo.Test Where SerialNumber='"
        ...
        private void Test()
        {
            ...
            BrushConverterIl.i = 0;
            BrushConverterRl.i = 0;
            Result = GetTestResult();
            if (IsQuickTest)
            {
                QuickTest();
